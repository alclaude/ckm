...
class AchievementPlottingCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
      ...
      # pathPlot =  pathDat
      $pathPlot = $resultRepository.$ID.'.end/'.$repPlot.'/'.$nameFilePlot;
      if( $fs->exists($pathPlot) ) {
              ...
              if($plotToRun){
                ...
                if( ! $hasResult ) {
                  ...
                  $message = \Swift_Message::newInstance()
                      ->setSubject('incoming plot analysis '.$nameFilePlot)
                      #->setFrom('ckmliveweb@gmail.com')
                      ->setFrom( array( 'ckmliveweb@in2p3.fr' => 'CKM Live Web' ) )
                      ->setTo($user->getEmail())
                      ->setBody($template->render('CKMUserBundle:Mail:plotNotification.txt.twig', array('user' => $user->getName(), 'plot' => $nameFilePlot, 'id' => $ID  ) ) )
                  ;
                  $mail->send($message);
                 /* 
        ...
        echo 'file '.$pathPlot. ' non existant'."\n";
      }
    }
}
